# -*- coding: utf-8 -*-
"""
Created on Thu Nov 11 17:19:30 2021
...
# Time2 = np.linspace(1996, 2000, 4) #this would be my x data

#%%

# Making a data frame to use for the 4 year rolling average. This is needed for pandas built in function
# print(Time)
# print(GuangxiData)

RollingData = pd.DataFrame({'Time':Time, 'Data':GuangxiData})
# print(RollingData)

RollingData['Rolling 4 Year'] = RollingData.iloc[:,1].rolling(window = 4).mean()

RollingData['Exponential Moving Average'] = RollingData.iloc[:,1].ewm(span = 4, adjust = False).mean()
# print(RollingData.head())

plt.figure
plt.plot(RollingData['Data'],label='Data')
plt.plot(RollingData['Exponential Moving Average'],label='Exp. Moving Avg.')
plt.plot(RollingData['Rolling 4 Year'],label='Rolling 4 Year Avg.')
plt.legend(loc=2)
...
N = np.average(GuangxiPopData * 10000) 
...
# fit model
results_infected = minimize(residual, params, args = (Time, GuangxiData), method = 'Nelder-Mead') 
# results_infected2 = minimize(residual, params, args = (Time2, GuangxiData2), method = 'Nelder-Mead')

# check results of the fit
data_fitted = GuangxiData + results_infected.residual.reshape(GuangxiData.shape)
#data_fitted2 = GuangxiData2 + results_infected2.residual.reshape(GuangxiData2.shape)
...
# #ax.plot(Time, GuangxiPopData * 10000, 'o')
# ax.plot(Time2, GuangxiData2, 'o')
# ax.plot(Time2, data_fitted2, '-', color = 'k', linewidth=2)
# #ax.plot(Time, pop_fitted, '-', color = 'k', linewidth=2) 
...
# #for spine in ('top', 'right', 'bottom', 'left'):
# #    ax.spines[spine].set_visible(False)
# plt.title("Guangxi (1996-2000)")
# plt.show()

#display fitted statistics
report_fit(results_infected)
# report_fit(results_infected2)
